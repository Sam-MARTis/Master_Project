...
class Point {
  ...
  addForceInteractionOfParticle = (other: Point): void => {
    ...
    if (this.force[0] > MAX_FORCE) this.force[0] = MAX_FORCE;
    if (this.force[1] > MAX_FORCE) this.force[1] = MAX_FORCE;
  };
  update = (dt: number): void => {
    ...
    if (this.x - this.size > widthMain) this.x = this.size;
    if (this.x + this.size < 0) this.x = widthMain - this.size;
    if (this.y - this.size > heightMain) this.y = this.size;
    if (this.y + this.size < 0) this.y = heightMain - this.size;
};
